...
# bola_fuego = Entity(model="quad", texture="assets/bola_fuego.png", scale=(0.5,0.5,0.5), position=(50,100,30),visible=True)

last_shot_time = 0  # Marca de tiempo del último disparo
shot_delay = 0.5  # Intervalo de tiempo entre disparos (en segundos)
...
enemy2 = Entity(model="quad", color = color.clear, scale =(1,1), position = (16,7), collider = "cube")
...
# Bola de fuego cuando se pulsa f para disparar 

def update(key):
    if key == "f" and (time.time() - last_shot_time) >= shot_delay:
        # Direcion de la bola fuego
        bola_fuego_star_position =(player.x + 1,player.y +1,0.4)

        # Crear la bala en la posición correcta
        new_bullet = bola_fuego( bola_fuego_star_position)
        new_bullet.parent = scene 

    last_shot_time = time.time() # Guardamos el tiempo del último disparo

class bola_fuego(Entity):
    def __init__(self, bola_fuego_star_position, direction):
        super().__init__()
        self.model = 'sphere'  # Usamos una esfera como modelo de la bola
        self.color = color.clear # Color de la bola
        self.scale = (0.5,0.5,0.5)   # Tamaño de la bola
        self.position = bola_fuego_star_position  # Posición inicial de la bola
        self.position = direction.normalized()  # Normalizamos la dirección para que sea un vector unitario
        self.speed = 6  # Velocidad de la bola

        self.bola_fuego_star_position = (100,100,100)
        self.direction.y = 0  # Reseteamos la componente Y de la dirección

def update(self):
    # Direcion de la bola de fuego 
    self.position += self.direction * self.speed * time.dt

    # Si la bola toca algun objeto se destrulla 
    if  bola_fuego.intersects(tuberia).hit  or bola_fuego.intersects(bloque):
        destroy(bola_fuego)
